...
begin
	...
	Ty = dy./ds
	Tz = dz./ds
	dTx = diff(Tx)./ds[2:end]
	dTy = diff(Ty)./ds[2:end]
	dTz = diff(Tz)./ds[2:end]
	ks = @. sqrt(dTx^2+dTy^2+dTz^2)
	Nx = dTx./ks
	Ny = dTy./ks
	Nz = dTz./ks
	Bx = Ty[2:end].*Nz .- Tz[2:end].*Ny
	By = Tz[2:end].*Nx .- Tx[2:end].*Nz
	Bz = Tx[2:end].*Ny .- Ty[2:end].*Nx
end	
...
begin
	plot(x,y,z)
	scatter!([x[s]],[y[s]],[z[s]])
	plot!([x[s],x[s]+Tx[s]],[y[s],y[s]+Ty[s]],[z[s],z[s]+Tz[s]],c=:black)
	plot!([x[s],x[s]+Nx[s]],[y[s],y[s]+Ny[s]],[z[s],z[s]+Nz[s]],c=:red)
	plot!([x[s],x[s]+Bx[s]],[y[s],y[s]+By[s]],[z[s],z[s]+Bz[s]],c=:blue)
	
end	

# ╔═╡ b99d33b8-aa82-49da-b6aa-3c3028f99eef
Ty[1]

# ╔═╡ 88de7c4a-7132-43dc-b87c-db11e787119c
begin
	g = -9.8
	gt = Tz*g
	gpx = -gt.*Tx
